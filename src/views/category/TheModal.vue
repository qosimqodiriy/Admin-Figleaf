...
import type { Category, CategoryForm, } from '@/services/category';
...
const data = reactive<{ display: boolean, formInfo: CategoryForm, error: boolean, message: string }>({
    message: '',
    error: false,
    display: false,
    formInfo: {
...
async function submit() {
    ...
    postPutCategory(data.formInfo).then((res: any) => {
...
<template>
    <Modal v-if="data.display">
      <div class="h-screen flex items-center justify-center fixed top-0 left-0 w-full p-10">
        <div class="bg-white-primary p-20 md:p-32 shadow-lg rounded-16 z-0 w-483 lg:w-550" :class="data.display ? 'z-0 scale-100 animate-blowUp' : ''">
          ...
          <form @submit.prevent="submit" class="w-full">
            <div class="flex flex-col md:flex-row md:items-end gap-20">
                <div class="w-100 h-120 rounded-5 border border-gray-secondary bg-white-secondary shrink-0">
