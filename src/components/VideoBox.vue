...
import { uploadVideo } from '@/services/common'
...
async function imageSelected(element: any) {
  ...
  await uploadVideo(formData).then((res: any) => {
    if(res[1] != null) {
      data.loading = false;
...
<template>
  <div>
    <div class="relative h-full w-full rounded flex items-center justify-center" @click="click">
      <video v-if="data.src" :src="`${getVideoURL}`" controls muted class="object-cover opacity-50 h-full w-full rounded"></video>
      <!-- <img v-if="data.src" :src="`${getVideoURL}`" alt="" class="object-cover opacity-50 h-full w-full rounded"> -->
    ...
    <input ref="inputRef" :onchange="imageSelected" name="video" @invalid="sendEmit" type="file" hidden accept="video/*">
  </div>
</template>
